...
export default {
    ...
    async mounted() {
        ...
        const summaryResponse = await fetch(`http://127.0.0.1:5000/api/v1/courses/${this.$route.params.course_id}/modules/${this.$route.params.module_id}/lessons/${this.$route.params.lesson_id}/generate-summary/slide`, {
            method: 'GET',
            headers: {
                'Authentication-Token': sessionStorage.getItem('auth-token')
            }
...
<template>
    <div id="slide-summarizer" class="rounded-3 ms-3 mt-2 p-3 bg-body-secondary">
