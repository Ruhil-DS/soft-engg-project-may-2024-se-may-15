...
def get_translation(source_text, target_language):
    prompt = f"""
        ...
        {target_language}. Make sure the meaning is not lost during the translation. If there certain
        domain specific words, like 'def' keyword in Python language, leave them in English.
        Here is the piece of text in English that needs to be translated to {target_language}:
        "{source_text}"
        
        Just return the translation as a JSON. Remember not to provide boilerplate text, no explanation,
        no unnecessary text to explain the output.
